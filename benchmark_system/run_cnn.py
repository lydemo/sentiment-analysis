...
base_dir = 'E:/stockcomment-preprocessed/'
data_dir = os.path.join(base_dir, 'stockcomment_trainfenci.txt')
label_dir = os.path.join(base_dir, 'label.txt')
embed_dir = os.path.join(base_dir, 'vectors_guba.txt')
# vocab_dir = os.path.join(base_dir, 'cnews.vocab.txt')
test_dir = 'E:/stockcomment-preprocessed/guba_commentfenci2.txt'
predict_dir = 'E:/stockcomment-preprocessed/predict.txt'

save_dir = 'E:/stockcomment-preprocessed/checkpoints/textcnn'
...
def batch_iter(x, y, batch_size=180):
    """生成批次数据"""
    data_len = len(x)
...
def batch_iter_x(x, batch_size=180):
    """生成批次数据"""
    data_len = len(x)
    num_batch = int((data_len - 1) / batch_size) + 1
...
def evaluate(sess, x_, y_):
    """评估在某一数据上的准确率和损失"""
    data_len = len(x_)
    batch_eval = batch_iter(x_, y_, 180)
    total_loss = 0.0
    total_acc = 0.0
...
def train(x_train, x_val, y_train, y_val, vocab_processor):
    ...
    # x_train, x_val, y_train, y_val = load_data_and_labels(data_dir, label_dir)
    ...
    if embed_dir:
        ...
        initW = np.random.uniform(-0.25,0.25,(len(vocab_processor.vocabulary_), config.embedding_dim))
...
def test():
    ...
    # with codecs.open(label_dir, 'r', encoding='utf8') as f:
    #     labels = f.readlines()
    # labels = np.array(labels, dtype='float') * 10
    # One-hot encoding
    # enc = preprocessing.OneHotEncoder()
    # y = enc.fit_transform(labels.reshape(len(labels), 1)).toarray()
    # Build vocabulary
    ...
    with codecs.open(predict_dir, 'w', encoding='utf8') as f:
        for label, content in zip(result, data):
            ...
            f.write("\n")
    # print (session.run(model.acc, feed_dict=feed_dict))
    # print('Testing...')
    # loss_test, acc_test = evaluate(session, x, y)
...
if __name__ == '__main__':
    ...
    if sys.argv[1] == 'train':
        train(x_train, x_val, y_train, y_val, vocab_processor)
    else:
        test()
